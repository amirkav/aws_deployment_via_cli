...
class CloudWatchLogsAgentSetup:
    ...
    def write_launcher_file(self):
        ...
        nicepath = subprocess.Popen(['which', 'nice'], stdout=subprocess.PIPE, universal_newlines=True).communicate()[
            0].rstrip()
    ...
    def get_distro_info(self):
        amazon_version_file_path = "/etc/system-release"
        redhat_version_file_path = "/etc/redhat-release"
        ubuntu_version_file_path = "/etc/issue"
        if os.path.isfile(amazon_version_file_path):
            issue_file_path = amazon_version_file_path
        elif os.path.isfile(redhat_version_file_path):
            issue_file_path = redhat_version_file_path
        elif os.path.isfile(ubuntu_version_file_path):
            issue_file_path = ubuntu_version_file_path
        else:
        ...
        if not os.path.exists(VIRTUALENV_ACTIVATE_CMD):
            ...
            if venv_in_path:
                ...
                if os.path.exists("/usr/bin/virtualenv"):
                    ...
                    fail(
                        "The path of the virtualenv command is unknown. Try manually installing with pip and adding it to the sudo user's PATH before running this script.",
                        MISSING_DEPENDENCY)
            if self.python:
            ...
            if venv_response != 0:
                fail(
                    "Failed to create virtualenv. Try manually installing with pip and adding it to the sudo user's PATH before running this script.",
                    MISSING_DEPENDENCY)
    ...
    def setup_initd(self):
        ...
        if self.os_flavor == self.Debian or self.os_flavor == self.Ubuntu:
            subprocess.call(['/usr/sbin/update-rc.d', 'awslogs', 'defaults'], env=DEFAULT_ENV, stderr=self.log_file,
                            stdout=self.log_file)
    ...
    def do_pip_install(self, package_descriptor, virtual_env_pip=True, folder_path=None, do_upgrade=False,
    ...
    def validate_dependency_path(self, path):
        if not os.path.exists(path):
            fail("Make sure the dependency_path points to the correct folder. Path doesn't exist: " + path,
                 STANDALONE_INSTALLATION_FAILURE)
    ...
    def setup_cron_jobs(self):
        nanny_cron_path = "/etc/cron.daily/awslogs"
        with open(nanny_cron_path, "w") as cron_fragment:
            cron_fragment.write(
                NANNY_CRON_CMD.format(VERSION=AWSLOGS_VERSION, AGENT_NANNY_PATH=AGENT_NANNY_PATH) + "\n")
        self.log_generated_file(nanny_cron_path)

        logrotate_cron_path = "/etc/cron.daily/awslogs_log_rotate"
    ...
    def write_nanny_script(self):
        ...
        ps_path = subprocess.Popen(['which', 'ps'], stdout=subprocess.PIPE, universal_newlines=True).communicate()[
            0].rstrip()
        grep_path = subprocess.Popen(['which', 'grep'], stdout=subprocess.PIPE, universal_newlines=True).communicate()[
            0].rstrip()
        cat_path = subprocess.Popen(['which', 'cat'], stdout=subprocess.PIPE, universal_newlines=True).communicate()[
            0].rstrip()
        service_path = \
        subprocess.Popen(['which', 'service'], stdout=subprocess.PIPE, universal_newlines=True).communicate()[
            0].rstrip()
    ...
    def write_awslogs_version_script(self):
        with open(AWSLOGS_VERSION_SCRIPT_PATH, "w") as version_script:
            version_script.write(
                AWSLOGS_VERSION_SCRIPT.replace("$AWSLOGS_GENERATED_FILES_LOG$", AWSLOGS_GENERATED_FILES_LOG))
    ...
    def setup_artifacts(self):
        ...
        with open(AWS_PROXY_CONFIG_FILE, "w") as config:
            config.write(
                PROXY_CONFIG.format(http_proxy=self.http_proxy, https_proxy=self.https_proxy, no_proxy=self.no_proxy,
...
def parse_args():
    parser = OptionParser()
    parser.add_option("-o", "--only-generate-config", dest="only_generate_config", action="store_true", default=False,
                      help="Only generate configuration without installing new bits.")
    parser.add_option("-n", "--non-interactive", dest="non_interactive", action="store_true", default=False,
    ...
    parser.add_option("--http-proxy", dest="http_proxy",
                      help="The http proxy that's used to communicate with CloudWatch Logs service.")
    parser.add_option("--https-proxy", dest="https_proxy",
                      help="The https proxy that's used to communicate with CloudWatch Logs service.")
    parser.add_option("--no-proxy", dest="no_proxy",
                      help="A comma-separated list of domain extensions the proxy should not be used for.")
    ...
    parser.add_option("--dependency-path", dest="dependency_path",
                      help="Path to local folder that has the dependency packages")
    parser.add_option("--ca-bundle", dest="ca_bundle",
                      help="Path for the CA certificate bundle to use for verifying SSL certificates.")
